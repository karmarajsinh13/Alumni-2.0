...
router.post("/",upload.single("photo"),async(req,res)=>{
    ...
    const {firstname,lastname,dob,email,phone,address,password,city,state,gender,passing_year,photo,username} = req.body;
    const photoFileName = req.file ? req.file.filename : null;
    const date = new Date();
    const AddUser = await db_function.AddUser(firstname,lastname,dob,email,phone,address,password,city,state,gender,passing_year,photoFileName,username,date)
    if(AddUser > 0){
